...
<head>
  ...
  @if ($blogParameter->favicon)
  <link rel="shortcut icon" href="{{ asset('storage/favicon/'. $blogParameter->favicon) }}" type="image/x-icon">
  <link rel="icon" href="{{ asset('storage/favicon/'. $blogParameter->favicon) }}" type="image/x-icon">
...
<script src="{{asset('assets/ticker/example/libs/jquery.tickerNews.min.js')}}"></script>
  ...
  $(".go-archive").click(function() {
          $('html, body').animate({
              scrollTop: $('#archive-area').offset().top
          }, 2000);
      });
        });
      </script>
...
{{-- <script type="text/javascript" src="{{asset('//s7.addthis.com/js/300/addthis_widget.js#pubid=')}}"></script> --}}

<script type="text/javascript" src="{!! asset($blogParameter->addthis_url) !!}"></script>
